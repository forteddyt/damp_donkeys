...
func ShowStudents(db *sql.DB, employername string) ([]Interview, error) {
    ...
    stmt, err := db.Prepare("SELECT displayname, major, class, DATE_FORMAT(CheckInTime, \"%h:%i\") FROM Interviews INNER JOIN StudentsCareerFairs USING (StudentID) WHERE EmployerID = (SELECT ID FROM Employers WHERE name = ?) AND CareerFairID = (SELECT MAX(CareerFairID) FROM CareerFairs);")
    if err != nil {
        return nil, err
    }
    defer stmt.Close()
