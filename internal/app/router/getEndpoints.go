...
func GetCareerFairStats(w http.ResponseWriter, r *http.Request){
	params := r.URL.Query()
	...
	numStudents, err := dbutil.GetNumberOfStudents(dbconn, params["career_fair_name"][0])

	// Database request error
	if err != nil {
		log.Printf("Could not get number of students from database: %s\n", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
	numInterviews, err := dbutil.GetNumberOfInterviews(dbconn, params["career_fair_name"][0])

	// Database request error
	if err != nil {
		log.Printf("Could not get number of interviews from database: %s\n", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	// Database request error
	if err != nil {
		log.Printf("Could not get number of companies from database: %s\n", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}


	resp := &resp.GetCareerFairStats {
		CompanyList: companies,
		StudentCount: numStudents,
		InterviewCount: numInterviews,
		JWT: new_jwt,
	}
...
func GetCompanyInterviewedList(w http.ResponseWriter, r *http.Request){
	params := r.URL.Query()
	...
	// Database request error
	if err != nil {
		log.Printf("Could not get career fair companies from database: %s\n", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	resp := &resp.GetCompanyInterviewedList {
		CompanyList: companies,
		JWT: new_jwt,
	}
	w.WriteHeader(http.StatusOK)
